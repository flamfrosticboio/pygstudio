...
def optimize_dlls(internal_folder):
    for dirpath, _, filenames in walk(internal_folder):
        if dirpath == internal_folder: continue
        for filename in filenames:
            if filename.endswith(".dll"): remove(join(dirpath, filename))
...
def release_item(source_folder, output_folder, other_args: str, flags):
    ...
    if exists(output_folder) and len(listdir(output_folder)) > 0 and flags.no_confirm == False:
        from . import shared
        shared.print_warning("Warning: The release folder has some contents that will be overwritten! Continue?")
        choice = shared.get_user_choice()
        if choice == False: return print("Operation is cancelled!") 
    
    rmtree(output_folder)
    game_name = basename(source_folder)
    temp_folder = join(output_folder, ".build")
    if not exists(temp_folder): makedirs(temp_folder)

    main_file = join(source_folder, "main.py")
    if not exists(main_file):
        main_file = join(source_folder, game_name+".py")
    if not exists(main_file):
        raise RuntimeError("Main file is not found. Consider renaming the main file to `main.py` or your <game_name>.py")
